...
fn test_derived_schema_performance() {
    ...
    assert!(
        cached_duration.as_millis() < 50,
        "Cached nested schema calls took too long: {:?}",
        cached_duration
    );
}
...
fn benchmark_derived_schema_generation() {
    ...
    println!(
        "  Simple struct ({} calls): {:?}",
        ITERATIONS, simple_duration
    );
    println!(
        "  Complex struct ({} calls): {:?}",
        ITERATIONS, complex_duration
    );
    println!(
        "  Nested struct ({} calls): {:?}",
        ITERATIONS, nested_duration
    );

    // All should be very fast due to caching
    assert!(
        simple_duration.as_millis() < 100,
        "Simple struct schema generation too slow: {:?}",
        simple_duration
    );
    assert!(
        complex_duration.as_millis() < 200,
        "Complex struct schema generation too slow: {:?}",
        complex_duration
    );
    assert!(
        nested_duration.as_millis() < 300,
        "Nested struct schema generation too slow: {:?}",
        nested_duration
    );
}
...
fn test_schema_caching_vs_regeneration() {
    ...
    assert!(
        avg_cached_call.as_micros() < 1000,
        "Cached calls should be very fast, got: {:?}",
        avg_cached_call
    );
    ...
    assert!(
        ...
        "1000 cached schema calls took too long: {:?}",
        cached_duration
    );
}
...
async fn test_schema_caching_in_tool_context() {
    ...
    assert!(
        ...
        "Tool schema generation with caching took too long: {:?}",
        duration
    );
}
