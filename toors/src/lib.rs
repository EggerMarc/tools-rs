//! lib/toors.rs
//! ============================================================
//! **Toors Runtime** – typed, JSON-driven function-calling for LLMs.
//!
//! If the **`schema`** Cargo-feature is enabled we embed real JSON-Schema
//! (generated by *schemars*) in every [`FunctionDecl`].  Otherwise those
//! fields are `null`, keeping the binary small and free of the extra dep.
//!
//! Build with full schema support: `cargo build --features schema`
...
// ---------------------------------------------------------------------------
//  ToolCollection – central registry
// ---------------------------------------------------------------------------
...
impl ToolCollection {
    ...
    /// Invoke a registered tool (used by the LLM dispatcher).
    ...
    /// Un-register a tool (mainly for tests).
    ...
    // ---------------------------------------------------------------------
    // Inventory-based auto-collection (for #[tool] macro)
    // ---------------------------------------------------------------------
    ...
    pub fn collect_tools() -> Self {
        ...
        for reg in inventory::iter::<ToolRegistration> {
            ...
            // schema-aware FunctionDecl ----------------------------------------
            hub.declarations.insert(
                reg.name,
                FunctionDecl {
                    name: reg.name,
                    description: reg.doc,
                    parameters: (reg.param_schema)(), // ◀ param   schema
                    returns: (reg.return_schema)(),   // ◀ return  schema
                },
            );
        }
